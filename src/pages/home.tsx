...
import { CAPTURE_SETTLE_MS, PRESET_SAVE_SETTLE_MS, VIDEO_WARMUP_MS } from '@/constants';
...
import { Alert, Button, Group, Modal, Progress, ScrollArea, SimpleGrid, Stack, Text, Tooltip } from '@mantine/core';
...
export function HomePage() {
  const { api, bridge, channel, endpoint, status, restartBridge, vcam, vcamBusy, toggleVcam } = useBridge();
  ...
  const stream = useVideoStream(endpoint, channel, status.connected && (controlsOpen || videoHold));
  ...
  if (bridge.status === 'failed') {
    return (
      <Stack>
        <Alert variant="light" color="red" title="Serviço de PTZ indisponível" icon={<IconAlertTriangle size={16} />}>
          <Text size="sm" mb="sm">
            {bridge.error}
          </Text>
  ...
  return (
    <>
        ...
        <div className={classes.scenes}>
          <Group className={classes.toolbar} justify="flex-end" gap="xs">
            ...
            <Tooltip label={vcamHint(vcam, channel)} withArrow multiline w={280}>
              <Button
                ...
                leftSection={vcam?.running ? <IconVideo size={18} /> : <IconVideoOff size={18} />}
                onClick={toggleVcam}
                loading={vcamBusy}
            ...
            <Button
              ...
              color="signalBlue"
              leftSection={controlsOpen ? <IconDeviceNintendoOff size={18} /> : <IconDeviceNintendo size={18} />}
            ...
            {capturing ? (
              ...
              <Tooltip label="Percorre os presets do canal e recaptura a miniatura de cada um" withArrow>
                <Button
                  size="xs"
          ...
          {capturing && progress && (
            <div className={classes.progress}>
              ...
              <Progress value={(progress.current / progress.total) * 100} size="sm" radius="xl" animated />
            </div>
          )}

          <ScrollArea className={classes.grid} type="auto" offsetScrollbars>
            <SimpleGrid cols={{ base: 2, xs: 3, sm: 4, lg: 5, xl: 6 }} spacing="sm" verticalSpacing="sm">
              {presets.map((preset) => (
                <PresetCard
                  key={preset.n}
                  preset={preset}
      ...
      <Modal opened={clearModal} onClose={() => setClearModal(false)} title="Limpar capturas" centered>
        <Text size="sm" mb="lg">
          Remover todas as miniaturas do canal {channel}? Os presets continuam gravados no equipamento — só as imagens
          são apagadas.
        </Text>
        <Group justify="flex-end">
          ...
          <Button color="red" leftSection={<IconEraser size={18} />} onClick={handleClearAll}>
            Sim, apagar todas
          </Button>
        </Group>
      </Modal>
      ...
      >
        <Text size="sm" mb="lg">
          Isso apaga a posição <strong>no equipamento</strong>, junto com o nome e a miniatura. Não dá para desfazer.
        </Text>
        <Group justify="flex-end">
          <Button variant="default" onClick={() => setDeleteTarget(null)}>
            Cancelar
          </Button>
          <Button color="red" leftSection={<IconTrash size={18} />} onClick={handleDelete}>
            Excluir preset
          </Button>
        </Group>
      </Modal>
